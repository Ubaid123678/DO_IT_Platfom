...
describe('KYC API HTTP Integration', () => {
  ...
  it('rejects invalid upload url payloads before service call', async () => {
    const token = generateAccessToken({
      sub: 'provider_2',
      role: 'provider',
    ...
    expect(mockedKycService.createUploadUrl).not.toHaveBeenCalled();
  });
